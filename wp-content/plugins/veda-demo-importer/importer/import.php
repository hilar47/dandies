...
class veda_import {
	...
	function import_content( $file = 'all.xml.gz' ){
		...
		$xml = WP_PLUGIN_DIR.'/veda-demo-importer/demos/'.$file;
	...
	function import_menu_location( $file = 'menu.txt' ){
		...
		$file_path 	= plugins_url ('veda-demo-importer') . '/demos/'. $file;
	...
	function import(){
		...
		if( class_exists( 'WP_Importer' ) && class_exists( 'WP_Import' ) ){
			...
			switch( $_REQUEST['data']['import'] ) {
				...
				default:
					// Empty select.import
					$this->error = esc_html__('Please select data to import.','veda-core');	
					break;
			}
			...
			if( $this->error ){
				...
				echo '<strong>'. esc_html__('All done. Have fun!','veda-core') .'</strong>';
			}
		}
		die(0);
	}
...
$dtthemes_import = new veda_import;
